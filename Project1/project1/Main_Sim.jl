...
# Refered Mykel's Textbook Pg. 124
...
function Find_Optimal_Graph(max_iters, df,annealing_thresh = 0.05)
    ...
    for i = 1:max_iters
        ...
        if(score_next>compute_bayesian_score(Data_Matrix,G_opt))
...
function compute(infile,name)
    ...
    # The Inputs are the the filename and the name(without file type) of the graph
    # The outputs are a .gph file, a .png file and a final bayesian score printed onto the terminal.
...
# The files are printed onto the same directory as the code itself.
